...
import com.faltenreich.release.ui.list.item.ListReleaseItem
...
class ReleaseDataSource(
...
) : PageKeyedDataSource<ReleaseKey, ListItem>() {
    private val releaseRepository = RepositoryFactory.repository<ReleaseRepository>()

    override fun loadInitial(params: LoadInitialParams<ReleaseKey>, callback: LoadInitialCallback<ReleaseKey, ListItem>) {
        load(startAt, params.requestedLoadSize, true, object : LoadCallback<ReleaseKey, ListItem>() {
            override fun onResult(data: MutableList<ListItem>, adjacentPageKey: ReleaseKey?) {
    ...
    override fun loadBefore(params: LoadParams<ReleaseKey>, callback: LoadCallback<ReleaseKey, ListItem>) {
        load(params.key, params.requestedLoadSize, false, callback)
    }

    override fun loadAfter(params: LoadParams<ReleaseKey>, callback: LoadCallback<ReleaseKey, ListItem>) {
        load(params.key, params.requestedLoadSize, true, callback)
    }

    private fun load(date: LocalDate, pageSize: Int, descending: Boolean, callback: LoadCallback<ReleaseKey, ListItem>) {
        ...
        releaseRepository.getBetween(start, end) { releases ->
            ...
            val items = dates.flatMap { date ->
                val dayItem = ListDateItem(date)
                val releasesOfDay = releases.filter { release -> release.releaseDate?.equals(date).isTrue }
                val releaseItems = releasesOfDay.mapNotNull { release -> release.releaseDate?.let { date -> ListReleaseItem(date, release) } }
