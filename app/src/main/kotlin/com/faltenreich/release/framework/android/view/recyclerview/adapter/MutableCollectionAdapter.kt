...
interface MutableCollectionAdapter <ITEM : Any> : CollectionAdapter<ITEM> {
    ...
    fun addListItemAt(position: Int, item: ITEM) = try { listItems.add(position, item) } catch (exception: IndexOutOfBoundsException) { Log.e(className, exception.toString()) }
    fun addListItems(items: List<ITEM>) = this.listItems.addAll(items)
    fun removeListItem(item: ITEM) = listItems.remove(item)
    fun removeListItemAt(position: Int) = try { listItems.removeAt(position) } catch (exception: IndexOutOfBoundsException) { Log.e(className, exception.toString()) }
